...
def run(_run, _config, _log):
    ...
    args = SN(**_config)
    args.device = "cuda:{}".format(args.device_index) if args.use_cuda else "cpu"
    ...
    if 'sia' in args.name:
        unique_token = "{}_bonus_weight_{}_aux_weight_{}_use_decay_{}_anneal_rate_{}_{}".format(
            args.name, args.bonus_weight, args.aux_weight, args.use_weight_decay, args.anneal_rate, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M"))
    else:
